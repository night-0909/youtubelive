# youtubelive
Automatic recording of youtube lives with chat with Mysql database.<br />
Plus, it can download files of video after live has ended, in case of you want a backup solution.

- recordytb.sql : creation of Mysql tables.<br />
- record_channel.py : record stream and chat of lives of a Youtube channel.<br />
For recording livestream, it uses yt-dlp (recommended) or streamlink to record stream. See https://github.com/yt-dlp/yt-dlp and  https://github.com/streamlink/streamlink<br />
For downloading files of video after live has ended, it uses yt-dlp.<br />
yt-dlp needs deno, install it and set its path in settings in 'yt-dlp_options'<br />
For chat, it uses chat_downloader. See https://github.com/xenova/chat-downloader<br />

Read documentation of these softwares and if an issue occurs search in their Github repository.<br />
...
Plus download files of video after livestream has ended.<br />
**Warning :** this script may mess up things in edge cases, so be cautious using it.
In settings, you can enable merge of video files, chat files renaming and downloading files of video.<br />
If you don't use record_process.py at all, you'll need to manually convert/merge all video files with ffmpeg for instance.
...
2 actions can be enabled in settings : record_video (record livestream), record_chat (record chat messages).<br />
...
It also gathers starttime of stream from Youtube API V3.<br />

- **record_process.py** : setup a cron every 10 min or something.<br />
3 actions can be enabled in settings : process_video (convert, merge, rename video files), process_chat (rename chat files) and download_files (download files of video).<br />
...
It can download files of video after live as ended.
It also gathers endtime of stream from Youtube API V3.<br />
...
For download files of video, clear status_downloading_all fields in same table.

**How to detect problems ?**<br />
See record_idchannel.log, yt-dlp_XXX.log, streamlink_XXX.log, chat_downloader_XXX.log, downloading_files_XXX.log, record_process.log and database records.
